...
export default class D3HierarchyEditor {
  ...
  drawNodes(source, instant = false) {
    ...
    function colorNode(node) {
      ...
      const hasHiddenChildren = (node._children?.length ?? 0) > 0;
      ...
      const dtypeColors = {
        ...
        none: "purple", // fallback
      };

      const dtypeColorsLight = {
        number: "#a6cee3",
        date: "#b2df8a",
        string: "#fccde5",
        determine: "#ffbb78",
    ...
    nodeEnter.each(function (d) {
      ...
      group.each(function (d) {
        ...
        if (isRoot) {
          // ⭐ Estrella SVG (5 puntas, centrada)
          group
            .append("circle")
            .attr("class", "showCircle")
            .attr("r", 12.5)
            .attr("fill", "black");
        } else if (isAggregation && !(nChildren === 0)) {
