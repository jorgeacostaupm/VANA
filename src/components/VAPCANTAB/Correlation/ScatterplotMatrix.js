...
export default function drawSPMatrix(data, config, parent, dimensions) {
  ...
  const { group, ...others } = data[0];
  const variables = Object.keys(others);
  ...
  function renderLinechart(var1, var2) {
    ...
    dots
      .enter()
      .append("circle")
      .attr("class", (d) => "dots " + "group" + d.group)
  ...
  function renderBigLineChart(var1, var2) {
    ...
    chart
      ...
      .attr("transform", `translate(-45, ${chartSize * 0.5}) rotate(-90)`)
      .attr("text-anchor", "middle")
      .attr("font-size", "18px")
      .text(var2);
