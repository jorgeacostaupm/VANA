...
import { addFilteringVariable } from "@/components/VAPUtils/features/compare/compareSlice";
...
export default class RankingPlot {
  ...
  updateVis() {
    ...
    vis.data = vis.data.filter(
      (d) => !vis.filterList.includes(d.variable) && d.pValue < vis.pValue
    );
  ...
  renderVis() {
    ...
    vis.xAxisG
      ...
      .on("mouseover", function (e, d) {
        const description = vis.descriptions[d]
          ? vis.descriptions[d]
