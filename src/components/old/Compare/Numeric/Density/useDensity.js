    ...
    if (blur) {
      legendItem
        ...
        .on("mousedown", (e) => {
          ...
          setBlur((prev) =>
            prev.includes(d) ? prev.filter((g) => g !== d) : [...prev, d]
          );

          chart
            .selectAll(".density")
            .filter((item) => item.group === d)
            .raise();
