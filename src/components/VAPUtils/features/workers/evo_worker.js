...
onmessage = function (event) {
  ...
  if (measure === "Z-Score")
    result = getEvolutionZscores(data, timeVar, pValue);
  else if (measure === "F-Value")
    result = getEvolutionFvalues(data, timeVar, pValue);
  else if (measure === "Chi-Square")
    result = getChiSquaredValues(data, timeVar, pValue);
  const msg = {
...
function getEvolutionZscores(groups, timeVar, pValue) {
  const z_scores = [];
  const pops = [];
  ...
  groups.forEach((group) => {
    ...
    /*     console.log(extremeVisits.map((d) => d[timeVar]));
    console.log(extremeVisits); */

    // Calcular Z-scores
    const group_z_scores = computeZscores(
      cleanedPopulation,
      timeVar,
      pValue
    ).map((d) => ({
...
function getChiSquaredValues(groups, timeVar, pValue) {
  const f_values = [];
  const pops = [];
  groups.forEach((group) => {
    const population = group[0];
    const tmp = group[1];
    const group_f_values = computeChiSquaredValues(tmp, timeVar, pValue).map(
      (d) => ({
        variable: d.variable + "^" + population,
        value: d.value,
        var: d.variable,
